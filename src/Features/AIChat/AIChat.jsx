...
function AIChat({ onHome, onSearch, onAbout, onFaq, onAdmin, onDashboard, onAIChat }) {
  ...
  const aiText = t?.ai || defaultAiText[language] || defaultAiText.he;

  // Chat messages history
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content: aiText.initial,
    },
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Display demo/error response in the selected language
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content: getDemoResponse(currentInput, language),
        },
      ]);
    } finally {
      setLoading(false);
